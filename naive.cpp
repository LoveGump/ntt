...
void NTT_parallel(uint64_t *a, uint64_t n, bool invert, uint64_t p, int g = 3) {
    ...
    for (int len = 2; len <= n; len <<= 1) {
        ...
        // 算单位根
        uint64_t g_n = invert ? 
                    pow(g, (p - 1) - (p - 1) / len, p) : pow(g, (p - 1) / len, p);
        ...
        for (int i = 0; i < NUM_THREADS; i++) {
            ...
            // 创建线程
            pthread_create(&threads[i], NULL, ntt_thread_function, &thread_args[i]);
        }
...
void NTT_multiply_parallel(uint64_t *a, uint64_t *b, uint64_t *result, int n, uint64_t p) {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        mul_args[i].p = p;
        pthread_create(&threads[i], NULL, multiply_thread_function, &mul_args[i]);
    }
