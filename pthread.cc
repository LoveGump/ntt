...
void parallel_reverse(uint64_t *a, int n, int bit) {
    ...
    // 计算逆序表
    for (int i = 0; i < n; i++) {
        task_state.rev[i] = (task_state.rev[i >> 1] >> 1) | ((i & 1) << (bit - 1));
    }
        ...
        case TASK_CRT_COMBINE: {
            ...
            for (int j = thread_id; j < result_length; j += NUM_THREADS) {
                ...
                for (int i = 0; i < mod_count; i++) {
                    ...
                    sum = (sum + (Mi * ((results[i][j] * Mi_inv) % mod_num))) % M;
                }
...
// 使用持久线程的NTT实现
void NTT_persistent(uint64_t *a, int n, bool invert, uint64_t p, int bit, int g = 3) {
    // 位逆序置换
...
// 使用CRT和持久线程的多项式乘法
void CRT_NTT_multiply_persistent(uint64_t *a, uint64_t *b, uint64_t *result, int n,
                                uint64_t p, pthread_t *threads) {
