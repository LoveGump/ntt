/**
 * 
 * 使用多线程 + MPI 来并行计算NTT，效果很好（比多进程好很多），但是修改的代码量不是很大
 */
...
// void fWrite1(uint64_t *ab, int n, std::string strout) {
//     std::string str1 = "files/";
//     std::string strout_full = str1 + strout;
//     char output_path[strout_full.size() + 1];
//     std::copy(strout_full.begin(), strout_full.end(), output_path);
//     output_path[strout_full.size()] = '\0';
//     std::ofstream fout;
//     fout.open(output_path, std::ios::out);
//     for (int i = 0; i < n * 2 - 1; i++) {
//         fout << ab[i] << '\n';
//     }
// }
// void fCheck1(uint64_t *ab, int n, std::string strout) {
//     std::string str1 = "files/";
//     std::string strout_full = str1 + strout;
//     char output_path[strout_full.size() + 1];
//     std::copy(strout_full.begin(), strout_full.end(), output_path);
//     output_path[strout_full.size()] = '\0';
//     std::ifstream fin;
//     fin.open(output_path, std::ios::in);
//     for (int i = 0; i < n * 2 - 1; i++) {
//         uint64_t x;
//         fin >> x;
//         if (x != ab[i]) {
...
// 预计算模数乘积及逆元相关值
__uint128_t GLOBAL_M = 1;
__uint128_t GLOBAL_MI_VALUES[4];
uint64_t GLOBAL_MI_INV_VALUES[4];
    ...
    while (true) {
        ...
        switch (task_state.current_task) {
            ...
            case TASK_CRT_COMBINE: {
                ...
                __uint128_t M = GLOBAL_M;
                __uint128_t *Mi_values = GLOBAL_MI_VALUES;
                uint64_t *Mi_inv_values = GLOBAL_MI_INV_VALUES;
                ...
                for (int j = start; j < end; j++) {
                    // 完全展开4个模数的循环
                    __uint128_t term0 = Mi_values[0] * ((results[0][j] * Mi_inv_values[0]) % GLOBAL_MOD_LIST[0]);
                    __uint128_t term1 = Mi_values[1] * ((results[1][j] * Mi_inv_values[1]) % GLOBAL_MOD_LIST[1]);
                    __uint128_t term2 = Mi_values[2] * ((results[2][j] * Mi_inv_values[2]) % GLOBAL_MOD_LIST[2]);
                    __uint128_t term3 = Mi_values[3] * ((results[3][j] * Mi_inv_values[3]) % GLOBAL_MOD_LIST[3]);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = test_begin; i <= test_end; ++i) {
        ...
        if (p_ > (1ULL << 32)) {
            ...
            // 对于小模数，使用pthread优化的NTT
            if (rank == 0) {
                Barrett br(p_);
                
                NTT_multiply_persistent(a, b, ab, n_, p_, threads, &br);
            }
        }
