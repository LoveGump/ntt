...
void NTT_Montgomerybase2(uint64_t *a, int n, bool invert, uint64_t p,Montgomery mont, int g = 3) {
...
void NTT_multiply_Montgomerybase2(uint64_t *a, uint64_t *b, uint64_t *result, int n, uint64_t p) {
    // 准备工作不变
    int len = 1;
    while (len < 2 * n) len <<= 1;
