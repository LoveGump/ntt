...
void NTT(uint64_t *a, int n, bool invert, uint64_t p, int g) {
    // 计算二进制位数
    int bit = 0;
    while ((1 << bit) < n)
        bit++;
...
void NTT_Montgomerybase2(uint64_t *a, int n, bool invert, uint64_t p,
                         Montgomery mont, int g = 3) {
    ...
    for (int len = 2; len <= n; len <<= 1) {
        ...
        // g_n^1
        uint64_t g_n_noamal = invert ? pow(g, (p - 1) - (p - 1) / len, p)
                                     : pow(g, (p - 1) / len, p);

        uint64_t g_n = mont.REDC(g_n_noamal * mont.R2);
        int step = len >> 1;
        // step 是当前的蝶形步长，step = len / 2
        for (int i = 0; i < n; i += len) {
            uint64_t g_pow = mont.R_mon_N; // 当前单位根幂次
            uint64_t u, v;
            for (int j = 0; j < step; j++) {
                u = a[i + j];
                // 使用Montgomery乘法
                v = mont.REDC(a[i + j + step] * g_pow);

                a[i + j] = (u + v) % p;
                a[i + j + step] = (u - v + p) % p;

                // 更新单位根
                g_pow = mont.REDC(g_pow * g_n);
            }
        }
    }
    ...
    if (invert) {
        ...
        uint64_t inv_n_mont = mont.REDC(inv_n * mont.R2);
...
void NTT_multiply_Montgomerybase2(uint64_t *a, uint64_t *b, uint64_t *result,
                                  int n, uint64_t p) {
    // 准备工作不变
    int len = 1;
    while (len < 2 * n)
